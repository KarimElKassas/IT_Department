...
class _SelectGroupUsersScreenState extends State<SelectGroupUsersScreen> {
  ...
  Widget selectedListItem(BuildContext context, NewGroupCubit cubit, state, int index) {
    return SlideInLeft(
  ...
  Widget _buildSearchField(NewGroupCubit cubit) {
    return Directionality(
      textDirection: TextDirection.rtl,
      child: Row(
        mainAxisSize: MainAxisSize.max,
        children: [
          ...
          Expanded(flex: 1, child: _buildActions(cubit))
        ],
      ),
    );
  }

  Widget _buildActions(NewGroupCubit cubit) {
